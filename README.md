...
4. Access the application.
   ...
   3. Test the endpoints: To test endpoint first you need to create token, without token you will not be able to access any endpoint.
   
   4. Curl to create token :
      `curl -X 'POST' \
   ...
   5. Once Token is generated, authorize the swagger-ui. You can click on authorize button and pass the token.
   
   6. Curl to fetch county by name :
       `curl -X 'GET' \
...
`
   7. Curl to fetch countries by different filters like population,area and language.
      `curl -X 'GET' \
